...
marketDashboardRouter.get('/market/recommendations/top10', async (_req,res,next)=>{try{
  ...
  const scored=stocks.map(stock=>{const result=scoreStock(stock),sector=scope.sectors.find(item=>stockMatchesSector(stock,item));return{...result,hotSector:sector?{rank:scope.sectors.indexOf(sector)+1,name:sector.name,score:sector.score}:null};}),eligible=scored.filter(x=>x.evidenceSufficient&&x.totalScore!=null).sort((a,b)=>b.totalScore-a.totalScore),items=eligible.slice(0,10);
  if(!items.length)return res.json({success:true,data:{method:{id:'hot-sector-trading-agents-v1'},hotSectors:scope.metadata,items:[],coverage:{sectorCount:scope.sectors.length,candidatesScored:stocks.length,evidenceEligible:0,returned:0,scope:'hot-sector-top3'},stage:`热门前三板块内评估 ${stocks.length} 只，但没有标的通过最低证据门槛；不跨板块补齐`,disclosure},meta:{source:'多源热点板块 + PostgreSQL audited data',status:'insufficient-evidence',mock:false,fetchedAt:new Date()}});
  res.json({success:true,data:{method:{id:'hot-sector-trading-agents-v1',stages:['热点板块证据排序','限定前三板块','技术与基本面筛选','多 Agent 复核','投委会排序'],roles:['market-agent','fundamentals-agent','news-agent','risk-agent','research-debate-agent'],aggregation:'热门板块限定后，四个专业角色可用分数均值 × 证据完整度',minimumEvidence:'至少3/4专业角色、20条日线、4条可引用证据；不足不进入Top10',candidateUniverse:'仅热门前三板块，按代码确定性截取最多400只；不足不跨板块补齐'},hotSectors:scope.metadata,items,coverage:{sectorCount:scope.sectors.length,eligibleUniverse,candidatesScored:stocks.length,evidenceEligible:eligible.length,returned:items.length,insufficientEvidence:scored.length-eligible.length,prefilterLimit:400,scope:'hot-sector-top3'},disclosure},meta:{source:'多源热点板块 + PostgreSQL audited market/financial/news data',status:items.length?'live':'insufficient-evidence',mock:false,fetchedAt:new Date(),dataAsOf:items.reduce((latest,x)=>!latest||new Date(x.asOf)>new Date(latest)?x.asOf:latest,null)}});
}catch(error){next(error);}});
