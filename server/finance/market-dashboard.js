...
import { getInStockSelection, storeInStockSelectionSnapshot } from './providers/instock.js';
...
marketDashboardRouter.post('/internal/instock/selection-sync',async(req,res,next)=>{try{const configured=process.env.INSTOCK_SYNC_TOKEN;if(!configured)throw Object.assign(new Error('INSTOCK_SYNC_TOKEN is not configured'),{status:503,code:'SYNC_NOT_CONFIGURED'});if(req.get('x-instock-sync-token')!==configured)throw Object.assign(new Error('Invalid sync token'),{status:401,code:'INVALID_SYNC_TOKEN'});const data=await storeInStockSelectionSnapshot(req.body||{});res.json({success:true,data:{dataDate:data.dataDate,syncedAt:data.syncedAt,coverage:data.coverage}});}catch(error){next(error);}});
...
marketDashboardRouter.get('/market/recommendations/top10', async (_req,res,next)=>{try{
  ...
  const db=getPrisma(),scope=await getTopSectorScope(3);
  if(!scope.sectors.length)return res.json({success:true,data:{method:{id:'hot-sector-trading-agents-v1',stages:['热点板块证据排序','限定前三板块','技术与基本面筛选','多 Agent 复核','投委会排序']},items:[],hotSectors:[],coverage:{sectorCount:0,candidatesScored:0,evidenceEligible:0,returned:0},stage:'热点板块证据不足，不跨板块补齐候选',disclosure},meta:{source:'多源新闻 + PostgreSQL audited data',status:'insufficient-evidence',mock:false,fetchedAt:new Date()}});
  const candidateWhere={status:'listed',prices:{some:{interval:'1d'}},...scope.where},[eligibleUniverse,stocks]=await Promise.all([db.stock.count({where:candidateWhere}),db.stock.findMany({where:candidateWhere,take:400,orderBy:{code:'asc'},include:{industry:true,prices:{where:{interval:'1d'},take:60,orderBy:{tradeDate:'desc'},include:{source:true}},statements:{take:4,orderBy:{periodEnd:'desc'},include:{source:true}},news:{take:20,orderBy:{publishedAt:'desc'},include:{source:true}}}})]);
  if(!stocks.length)return res.json({success:true,data:{method:{id:'hot-sector-trading-agents-v1'},hotSectors:scope.metadata,items:[],coverage:{sectorCount:scope.sectors.length,candidatesScored:0,evidenceEligible:0,returned:0,scope:'hot-sector-top3'},stage:'热门前三板块内暂无已入库日线候选，不跨板块补齐',disclosure},meta:{source:'多源热点板块 + PostgreSQL audited data',status:'insufficient-evidence',mock:false,fetchedAt:new Date()}});
  const scored=stocks.map(stock=>{const result=scoreStock(stock),sector=scope.sectors.find(item=>stockMatchesSector(stock,item));return{...result,hotSector:sector?{rank:scope.sectors.indexOf(sector)+1,name:sector.name,score:sector.score}:null};}),evidenceEligible=scored.filter(x=>x.evidenceSufficient&&x.totalScore!=null),eligible=evidenceEligible.filter(x=>x.totalScore>=58).sort((a,b)=>b.totalScore-a.totalScore),items=eligible.slice(0,10);
  if(!items.length)return res.json({success:true,data:{method:{id:'hot-sector-trading-agents-v1',minimumScore:58},hotSectors:scope.metadata,items:[],coverage:{sectorCount:scope.sectors.length,eligibleUniverse,candidatesScored:stocks.length,evidenceEligible:evidenceEligible.length,recommendationEligible:0,returned:0,scope:'hot-sector-top3'},stage:`热门前三板块内评估 ${stocks.length} 只，${evidenceEligible.length} 只证据完整，但没有标的达到58分观察门槛；不跨板块补齐`,disclosure},meta:{source:'多源热点板块 + PostgreSQL audited data',status:'insufficient-evidence',mock:false,fetchedAt:new Date()}});
  res.json({success:true,data:{method:{id:'hot-sector-trading-agents-v1',stages:['热点板块证据排序','限定前三板块','技术与基本面筛选','多 Agent 复核','投委会排序'],roles:['market-agent','fundamentals-agent','news-agent','risk-agent','research-debate-agent'],aggregation:'热门板块限定后，四个专业角色可用分数均值 × 证据完整度',minimumEvidence:'至少3/4专业角色、20条日线、4条可引用证据，且综合分≥58；不足不进入Top10',minimumScore:58,candidateUniverse:'仅热门前三板块，按代码确定性截取最多400只；不足不跨板块补齐'},hotSectors:scope.metadata,items,coverage:{sectorCount:scope.sectors.length,eligibleUniverse,candidatesScored:stocks.length,evidenceEligible:evidenceEligible.length,recommendationEligible:eligible.length,returned:items.length,insufficientEvidence:scored.length-evidenceEligible.length,prefilterLimit:400,scope:'hot-sector-top3'},disclosure},meta:{source:'多源热点板块 + PostgreSQL audited market/financial/news data',status:items.length?'live':'insufficient-evidence',mock:false,fetchedAt:new Date(),dataAsOf:items.reduce((latest,x)=>!latest||new Date(x.asOf)>new Date(latest)?x.asOf:latest,null)}});
}catch(error){next(error);}});
...
marketDashboardRouter.get('/market/scans/technical', async(req,res,next)=>{try{
  ...
  const db=getPrisma(),limit=Math.min(100,Math.max(1,Number(req.query.limit)||30)),scope=await getTopSectorScope(3);
  if(!scope.sectors.length)return res.json({success:true,data:{hotSectors:[],categories:{up3to5:[],macdGoldenCross:[],threeWhiteSoldiers:[]},coverage:{universe:'热门板块证据不足',listed:0,withDailyHistory:0,macdEligible:0,threeDayEligible:0,note:'不跨板块扫描。'}},meta:{mock:false,status:'insufficient-evidence',updatedAt:new Date()}});
  const technicalWhere={status:'listed',prices:{some:{interval:'1d'}},...scope.where},[eligibleUniverse,scoped]=await Promise.all([db.stock.count({where:technicalWhere}),db.stock.findMany({where:technicalWhere,take:400,orderBy:{code:'asc'},select:{id:true}})]),tagged=[];let macdEligible=0,threeDayEligible=0;
  for(let offset=0;offset<scoped.length;offset+=150){const ids=scoped.slice(offset,offset+150).map(item=>item.id),batch=await db.stock.findMany({where:{id:{in:ids}},include:{industry:true,prices:{where:{interval:'1d'},take:60,orderBy:{tradeDate:'desc'},include:{source:true}}}});for(const stock of batch){if(stock.prices.length>=35)macdEligible++;if(stock.prices.length>=3)threeDayEligible++;const sector=scope.sectors.find(item=>stockMatchesSector(stock,item));tagged.push(...signals(stock).map(signal=>({...signal,industry:stock.industry?.name||null,hotSector:sector?{rank:scope.sectors.indexOf(sector)+1,name:sector.name,score:sector.score}:null})));}}
  const grouped={up3to5:tagged.filter(x=>x.type==='up_3_5').slice(0,limit),macdGoldenCross:tagged.filter(x=>x.type==='macd_golden_cross').slice(0,limit),threeWhiteSoldiers:tagged.filter(x=>x.type==='three_white_soldiers').slice(0,limit)};
  res.json({success:true,data:{hotSectors:scope.metadata,categories:grouped,coverage:{universe:'热门前三板块',sectorCount:scope.sectors.length,eligibleUniverse,listed:scoped.length,withDailyHistory:scoped.length,macdEligible,threeDayEligible,notCovered:Math.max(0,eligibleUniverse-scoped.length),candidateLimit:400,note:'仅在热点证据得分最高的三个板块内，按代码确定性选取最多400只并分批扫描真实日线；不足不跨板块补齐。'},definitions:{up3to5:'最新日涨幅闭区间[3%,5%]',macdGoldenCross:'当日DIF上穿DEA，EMA(12,26,9)',threeWhiteSoldiers:'连续三根阳线且收盘价逐日抬高；三红兵/三武士按用户同义称呼归一'}},meta:{mock:false,status:'live',updatedAt:new Date()}});
}catch(error){next(error);}});
