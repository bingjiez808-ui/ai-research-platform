...
marketDashboardRouter.get('/market/recommendations/top10', async (_req,res,next)=>{try{
  ...
  const db=getPrisma(),candidateRows=await db.$queryRaw`SELECT sp.stock_id AS id FROM stock_prices sp JOIN stocks s ON s.id=sp.stock_id WHERE s.status='listed' AND sp.interval='1d' GROUP BY sp.stock_id HAVING COUNT(DISTINCT sp.trade_date)>=20 ORDER BY MAX(sp.trade_date) DESC LIMIT 400`,candidateIds=candidateRows.map(row=>row.id);
  const stocks=candidateIds.length?await db.stock.findMany({where:{id:{in:candidateIds}},include:{industry:true,prices:{where:{interval:'1d'},take:60,orderBy:{tradeDate:'desc'},include:{source:true}},statements:{take:4,orderBy:{periodEnd:'desc'},include:{source:true}},news:{take:20,orderBy:{publishedAt:'desc'},include:{source:true}}}}):[];
...
marketDashboardRouter.get('/market/scans/technical', async(req,res,next)=>{try{
  ...
  const [listed,historyRows,upRows,soldierRows]=await Promise.all([
    db.stock.count({where:{status:'listed'}}),
    db.$queryRaw`SELECT sp.stock_id AS id, COUNT(DISTINCT sp.trade_date)::int AS days FROM stock_prices sp JOIN stocks s ON s.id=sp.stock_id WHERE s.status='listed' AND sp.interval='1d' GROUP BY sp.stock_id`,
    db.$queryRaw`SELECT code,name,trade_date,change_percent FROM (SELECT DISTINCT ON (s.id) s.code,s.name,sp.trade_date,sp.change_percent FROM stocks s JOIN stock_prices sp ON sp.stock_id=s.id WHERE s.status='listed' AND sp.interval='1d' ORDER BY s.id,sp.trade_date DESC) latest WHERE change_percent BETWEEN 3 AND 5 ORDER BY change_percent DESC LIMIT 100`,
    db.$queryRaw`WITH recent AS (SELECT s.code,s.name,sp.trade_date,sp.open,sp.close,ROW_NUMBER() OVER(PARTITION BY s.id ORDER BY sp.trade_date DESC) rn FROM stocks s JOIN stock_prices sp ON sp.stock_id=s.id WHERE s.status='listed' AND sp.interval='1d') SELECT code,name,MAX(trade_date) trade_date,array_agg(open ORDER BY trade_date) opens,array_agg(close ORDER BY trade_date) closes FROM recent WHERE rn<=3 GROUP BY code,name HAVING COUNT(*)=3 AND bool_and(close>open) AND (array_agg(close ORDER BY trade_date))[1]<(array_agg(close ORDER BY trade_date))[2] AND (array_agg(close ORDER BY trade_date))[2]<(array_agg(close ORDER BY trade_date))[3] LIMIT 100`
  ]);
  const macdIds=historyRows.filter(row=>row.days>=35).map(row=>row.id),macdStocks=macdIds.length?await db.stock.findMany({where:{id:{in:macdIds}},include:{prices:{where:{interval:'1d'},take:60,orderBy:{tradeDate:'desc'},include:{source:true}}}}):[],macdSignals=macdStocks.flatMap(signals).filter(x=>x.type==='macd_golden_cross');
  const grouped={up3to5:upRows.slice(0,limit).map(row=>({type:'up_3_5',code:row.code,name:row.name,latestTradeDate:row.trade_date,evidence:{changePercent:asNumber(row.change_percent),tradeDate:row.trade_date}})),macdGoldenCross:macdSignals.slice(0,limit),threeWhiteSoldiers:soldierRows.slice(0,limit).map(row=>({type:'three_white_soldiers',aliases:['三红兵','三武士'],code:row.code,name:row.name,latestTradeDate:row.trade_date,evidence:{opens:row.opens.map(asNumber),closes:row.closes.map(asNumber),definition:'连续三根阳线且收盘价逐日抬高'}}))},withDailyHistory=historyRows.length;
  res.json({success:true,data:{categories:grouped,coverage:{universe:'PostgreSQL status=listed',listed,withDailyHistory,macdEligible:macdIds.length,threeDayEligible:historyRows.filter(x=>x.days>=3).length,notCovered:listed-withDailyHistory,note:'涨幅与三红兵由 PostgreSQL 集合扫描；仅对35日以上标的在服务端计算MACD，避免全市场数据同时占用内存。'},definitions:{up3to5:'最新日涨幅闭区间[3%,5%]',macdGoldenCross:'当日DIF上穿DEA，EMA(12,26,9)',threeWhiteSoldiers:'连续三根阳线且收盘价逐日抬高；三红兵/三武士按用户同义称呼归一'}},meta:{mock:false,updatedAt:new Date()}});
}catch(error){next(error);}});
