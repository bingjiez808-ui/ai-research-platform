...
marketDashboardRouter.get('/market/recommendations/top10', async (_req,res,next)=>{try{
  ...
  const stocks=await db.stock.findMany({where:{status:'listed',prices:{some:{interval:'1d'}},...scope.where},take:400,include:{industry:true,prices:{where:{interval:'1d'},take:60,orderBy:{tradeDate:'desc'},include:{source:true}},statements:{take:4,orderBy:{periodEnd:'desc'},include:{source:true}},news:{take:20,orderBy:{publishedAt:'desc'},include:{source:true}}}});
...
marketDashboardRouter.get('/market/scans/technical', async(req,res,next)=>{try{
  ...
  const scoped=await db.stock.findMany({where:{status:'listed',prices:{some:{interval:'1d'}},...scope.where},take:1500,select:{id:true}}),tagged=[];let macdEligible=0,threeDayEligible=0;
