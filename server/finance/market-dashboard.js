...
marketDashboardRouter.get('/market/scans/technical', async(req,res,next)=>{try{
  ...
  const [listed,historyRows]=await Promise.all([db.stock.count({where:{status:'listed'}}),db.$queryRaw`SELECT sp.stock_id AS id, COUNT(DISTINCT sp.trade_date)::int AS days FROM stock_prices sp JOIN stocks s ON s.id=sp.stock_id WHERE s.status='listed' AND sp.interval='1d' GROUP BY sp.stock_id`]);
  const all=[];for(let offset=0;offset<historyRows.length;offset+=250){const ids=historyRows.slice(offset,offset+250).map(row=>row.id),batch=await db.stock.findMany({where:{id:{in:ids}},include:{prices:{where:{interval:'1d'},take:60,orderBy:{tradeDate:'desc'},include:{source:true}}}});all.push(...batch.flatMap(signals));}
  const grouped={up3to5:all.filter(x=>x.type==='up_3_5').slice(0,limit),macdGoldenCross:all.filter(x=>x.type==='macd_golden_cross').slice(0,limit),threeWhiteSoldiers:all.filter(x=>x.type==='three_white_soldiers').slice(0,limit)},withDailyHistory=historyRows.length;
  res.json({success:true,data:{categories:grouped,coverage:{universe:'PostgreSQL status=listed',listed,withDailyHistory,macdEligible:historyRows.filter(x=>x.days>=35).length,threeDayEligible:historyRows.filter(x=>x.days>=3).length,notCovered:listed-withDailyHistory,note:'按250只分批扫描已入库真实日线，避免全市场数据同时占用内存；不以近似条件冒充MACD或K线形态。'},definitions:{up3to5:'最新日涨幅闭区间[3%,5%]',macdGoldenCross:'当日DIF上穿DEA，EMA(12,26,9)',threeWhiteSoldiers:'连续三根阳线且收盘价逐日抬高；三红兵/三武士按用户同义称呼归一'}},meta:{mock:false,updatedAt:new Date()}});
}catch(error){next(error);}});
