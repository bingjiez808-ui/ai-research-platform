...
marketDashboardRouter.get('/market/scans/technical', async(req,res,next)=>{try{
  ...
  const stocks=await db.stock.findMany({where:{status:'listed',prices:{some:{interval:'1d'}},...scope.where},take:1500,include:{industry:true,prices:{where:{interval:'1d'},take:60,orderBy:{tradeDate:'desc'},include:{source:true}}}}),tagged=stocks.flatMap(stock=>signals(stock).map(signal=>{const sector=scope.sectors.find(item=>stockMatchesSector(stock,item));return{...signal,industry:stock.industry?.name||null,hotSector:sector?{rank:scope.sectors.indexOf(sector)+1,name:sector.name,score:sector.score}:null};})),grouped={up3to5:tagged.filter(x=>x.type==='up_3_5').slice(0,limit),macdGoldenCross:tagged.filter(x=>x.type==='macd_golden_cross').slice(0,limit),threeWhiteSoldiers:tagged.filter(x=>x.type==='three_white_soldiers').slice(0,limit)};
  res.json({success:true,data:{hotSectors:scope.metadata,categories:grouped,coverage:{universe:'热门前三板块',sectorCount:scope.sectors.length,listed:stocks.length,withDailyHistory:stocks.length,macdEligible:stocks.filter(x=>x.prices.length>=35).length,threeDayEligible:stocks.filter(x=>x.prices.length>=3).length,notCovered:0,note:'仅在热点证据得分最高的三个板块内扫描真实日线；不足不跨板块补齐。'},definitions:{up3to5:'最新日涨幅闭区间[3%,5%]',macdGoldenCross:'当日DIF上穿DEA，EMA(12,26,9)',threeWhiteSoldiers:'连续三根阳线且收盘价逐日抬高；三红兵/三武士按用户同义称呼归一'}},meta:{mock:false,status:'live',updatedAt:new Date()}});
}catch(error){next(error);}});
