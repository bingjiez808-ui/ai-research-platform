...
dailyAgentRouter.get('/daily-agent/latest',async(req,res,next)=>{try{const data=await getPrisma().dailyAgentRun.findFirst({where:{ownerKey:ownerKey(req),status:'succeeded'},orderBy:{runDate:'desc'},include:{reports:{orderBy:{generatedAt:'desc'},include:{stock:{select:{code:true,name:true}}}}}});if(!data)return res.status(404).json({success:false,error:{code:'REPORT_NOT_FOUND',message:'No completed daily report'}});res.json({success:true,data});}catch(e){next(e);}});
