...
function evaluate(stock,strategy){
  const p=stock.prices[0],f=stock.statements[0],marketCap=number(p?.marketCap),values={roe:number(f?.roe??p?.raw?.roe),pe:number(p?.pe),pb:number(p?.pb),marketCapYi:marketCap==null?null:marketCap/1e8,changePercent:number(p?.changePercent),turnoverRate:number(p?.turnoverRate)},gaps=[];
  let sum=0,weight=0;
  for(const [key,target] of Object.entries(strategy.rules)){const def=RULES[key],actual=values[def.field];weight++;if(!Number.isFinite(actual)){gaps.push({rule:key,label:def.label,target,actual:null,status:'missing',gap:null});continue;}const economicallyValid=key!=='maxPe'||actual>0,pass=economicallyValid&&(def.mode==='min'?actual>=target:actual<=target),den=Math.max(Math.abs(target),1),distance=pass?0:economicallyValid?Math.abs(actual-target)/den:1,score=clamp(100-distance*100);sum+=score;gaps.push({rule:key,label:def.label,target,actual:Number(actual.toFixed(2)),status:pass?'pass':'near',gap:economicallyValid?Number(Math.abs(actual-target).toFixed(2)):null,reason:economicallyValid?undefined:'亏损或无有效正市盈率'});}
...
export async function evaluateStrategy(input){
  ...
  const db=getPrisma(),candidateWhere={status:'listed',prices:{some:{interval:'1d'}},...scope.where},[eligibleUniverse,stocks]=await Promise.all([db.stock.count({where:candidateWhere}),db.stock.findMany({where:candidateWhere,take:400,orderBy:{code:'asc'},include:{industry:true,prices:{where:{interval:'1d'},take:1,orderBy:{tradeDate:'desc'},include:{source:true}},statements:{take:1,orderBy:{periodEnd:'desc'},include:{source:true}}}})]);
  ...
  return{strategy,hotSectors:scope.metadata,items,optimization:{suggestions,method:'先限定热门前三板块，按代码确定性选取最多400只，再按用户规则距离与数据完整度排序；不修改用户硬约束，不执行用户代码。'},coverage:{sectorCount:scope.sectors.length,eligibleUniverse,candidateLimit:400,listedWithPrice:stocks.length,evaluated:ranked.length,returned:items.length,scope:'hot-sector-top3'},asOf:new Date(),disclosure:'候选仅表示热门前三板块内最接近用户规则，不是买入建议；接口不承诺次日表现。'};
}
