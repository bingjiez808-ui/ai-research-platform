...
export function normalizeSelectionRow(row) {
  ...
  return {code,name:first(row,['name','stock_name'])||code,date:dateText(first(row,['date','trade_date'])),industry:first(row,['industry']),concept:first(row,['concept']),style:first(row,['style']),quote:{price,changePercent,turnoverRate,volumeRatio,pe,pb},fundamentals:{roe,roic,grossMargin,netMargin,revenueGrowth,profitGrowth,cashFlow,debtRatio,peg,predictedGrowth},technicalEvidence:{macdDaily:flag(row,['macd_golden_fork'])===100,macdWeekly:flag(row,['macd_golden_fork_week'])===100,macdMonthly:flag(row,['macd_golden_fork_month'])===100,breakout:flag(row,['break_through'])===100,aboveMa20:flag(row,['breakup_ma_20days'])===100,longMaAlignment:flag(row,['long_avg_array'])===100,volumeRatio,turnoverRate,changePercent},factorScores:Object.fromEntries(Object.entries(factorScores).map(([key,value])=>[key,value==null?null:Number(value.toFixed(1))])),totalScore:totalScore==null?null:Number(totalScore.toFixed(1)),evidenceCompleteness:Number((evidenceCount/10).toFixed(2))};
}
...
export async function storeInStockSelectionSnapshot(payload){
  const rows=unwrapInStockRows(payload).slice(0,5000),normalized=rows.map(normalizeSelectionRow).filter(item=>item.code),ranked=normalized.filter(item=>item.totalScore!=null).sort((a,b)=>b.totalScore-a.totalScore),fields=['quality','growthValuation','technical','marketChip','risk'],dataDate=ranked.map(item=>item.date).filter(Boolean).sort().at(-1)||dateText(payload?.dataDate)||new Date().toISOString().slice(0,10),snapshot={dataDate,items:ranked.slice(0,500),coverage:{rawRows:rows.length,normalized:normalized.length,scored:ranked.length,averageCompleteness:ranked.length?Number((ranked.reduce((sum,item)=>sum+item.evidenceCompleteness,0)/ranked.length).toFixed(2)):0,factors:Object.fromEntries(fields.map(field=>[field,normalized.filter(item=>item.factorScores[field]!=null).length]))},syncedAt:new Date().toISOString()};
