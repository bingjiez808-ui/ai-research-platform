...
export async function runTradingWorkflow(code,{mode='auto'}={}) {
  ...
  const sentimentValues=stock.news.map(row=>/增长|回购|增持|中标|突破/.test(row.title)?1:/亏损|减持|处罚|调查|诉讼|下滑|风险/.test(row.title)?-1:0),sentimentScore=sentimentValues.length?clamp(50+sentimentValues.reduce((a,b)=>a+b,0)/sentimentValues.length*30):(return20==null?null:clamp(50+return20*.4));
  const financial=stock.statements[0],revenue=Number(financial?.revenue),netProfit=Number(financial?.netProfit),cashFlow=Number(financial?.operatingCashFlow),roe=Number(financial?.roe),margin=Number.isFinite(revenue)&&revenue>0&&Number.isFinite(netProfit)?netProfit/revenue*100:null,fundamentalScore=!financial?null:clamp(Number.isFinite(roe)&&roe!==0?50+roe*1.8:45+(margin==null?0:Math.max(-15,Math.min(25,margin*.8)))+(cashFlow>0?10:cashFlow<0?-12:0)),marketScore=market.score,scoreBreakdown={technical:technicalScore,sentiment:sentimentScore,market:marketScore,fundamental:fundamentalScore},compositeScore=weightedScore(scoreBreakdown);
