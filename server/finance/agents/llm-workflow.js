...
export async function runLlmTradingWorkflow(stock,{call=callFinanceAgent}={}){
  ...
  const proposedAction=guardPassed?(portfolio.action||portfolio.stance||'watch'):'insufficient-evidence',scoreBreakdown={technical:num(market.score),market:num(market.score),fundamental:num(fundamental.score),sentiment:num(news.score)},compositeScore=weightedScore(scoreBreakdown),tradePlan=buildTradePlan(stock.prices,compositeScore??50,{evidenceSufficient:guardPassed}),action=proposedAction==='research-candidate'&&tradePlan.status==='ready'?tradePlan.action:proposedAction;
  const recommendationSummary=buildRecommendationSummary(scoreBreakdown,num(risk.score),tradePlan,{evidenceSufficient:guardPassed});
  return {workflow:'TradingAgents LLM evidence workflow',version:'llm-debate-1.1.0',executionMode:'llm',configured:true,code:stock.code,name:stock.name,asOf:new Date(),action,confidence:Number(portfolio.confidence||0),scoreBreakdown,compositeScore,tradePlan,recommendationSummary,finalDecision:{action,label:decisionLabel(action),summary:portfolio.summary||portfolio.reason||'投资委员会已综合技术、情绪、市场与基本面证据。'},evidenceCoverage:{historyCount,financialCount:groups.fundamental.length,newsCount:groups.news.length,citations:cited.length,guardPassed},analysts:analystViews,debate:{bull,bear},risk,portfolioManager:{...portfolio,action},trace:['market-analyst','fundamentals-analyst','news-analyst','bull-researcher','bear-researcher','risk-manager','portfolio-manager','deterministic-evidence-guard','conditional-price-plan'],disclosure:'由配置的外部大模型基于已入库证据真实运行；买卖价由透明波动规则生成，模型可能出错，不构成投资建议。'};
}
