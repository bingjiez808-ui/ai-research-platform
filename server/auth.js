...
function setCookie(res,token,maxAge=TTL){res.append('Set-Cookie',`${COOKIE}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${Math.floor(maxAge/1000)}${process.env.NODE_ENV==='production'?'; Secure':''}`);}
